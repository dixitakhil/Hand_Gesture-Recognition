...
for directory, subdirectories, files in os.walk(root): # Parse each  directory in the  folder
    ...
    for file in files: # Parse each file in that directory
        ...
        value = im.flatten()
        value = np.hstack((directory[8:],value))  #The directory[11:] indicates after which character the class name should start so chnage it according to the file name. Hstack places array in stack on each other and directory name is made first coloumn
